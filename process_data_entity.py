...
def convert_mm_examples_to_features(examples, label_list, auxlabel_list, max_seq_length, tokenizer, crop_size, path_img):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        image_paths = [os.path.join(path_img, "{}/{}_{}.jpg".format(image_name, entity, image_name))
        ...
        for temp_image_path in image_paths:
            ...
            try:
                image.append(image_process(temp_image_path, transform))
            except Exception as e:
                count += 1
                print('image has problem：' + str(e))   #some image sizes are smaller than crop size 224*224
        image_path_fail = os.path.join('../twit_subimages/twitter2017_images/', '17_06_4705.jpg')
        image_path_fail = image_process(image_path_fail, transform)
        for i in range(4-len(image)):
            image.append(image_path_fail)

        #image = torch.stack([f for f in image])
        image = sum(image)/4
        # if ex_index < 2:
        #     logger.info("*** Example ***")
        #     logger.info("guid: %s" % (example.guid))
