...
from process_data_entity import *
...
def main():
    ...
    if args.task_name == "twitter2017":
        args.path_image = args.path_image + "/twitter2017_entities/"
    elif args.task_name == "twitter2015":
        args.path_image = args.path_image + "/twitter2015_entities/"
    ...
    if args.do_train:
        ...
        for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                with torch.no_grad():
                    _, _, temp_att0 = encoder(img_feats[:,0])
                    _, _, temp_att1 = encoder(img_feats[:,1])
                    _, _, temp_att2 = encoder(img_feats[:, 2])
                    _, _, temp_att3 = encoder(img_feats[:, 3])
                    img_att = (temp_att0+temp_att1+temp_att2+temp_att3)/4
